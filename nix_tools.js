...
// 
// 
export async function ensureInstalled(options={defaultVersion: null}) {
...
export function jsonValueToNix(obj) {
    // 
    // custom
    // 
        ...
        // 
            ...
            if (entries.length == 0) {
                ...
                for (const [key, value] of entries) {
                    const valueAsString = this(value)
                    const valueIsSingleLine = !valueAsString.match(/\n/)
                    if (valueIsSingleLine) {
                        string += indent({
                            string: `${escapeNixString(key)} = ${this(value)};`
                        }) + "\n"
                    } else {
                        string += indent({
...
jsonValueToNix = jsonValueToNix.bind(jsonValueToNix)
jsonValueToNix._customJsConverters = []
jsonValueToNix.addCustomJsConverter = (function({checker, converter}) {
...
const parser = await parserFromWasm(nixTreeSitter) // argument is path or Uint8Array
...
const nixJsRuntime = ()=>{
    const builtins = Object.freeze({
        ...
        "isPath": ()=>{/*FIXME*/},
        "isString": (value)=>typeof value == "string", // TODO: might need to change for lazy interpolated strings
        ...
        "appendContext": ()=>{/*FIXME*/},
        "attrNames": (value)=>Object.keys(value).sorted(), // FIXME make sure JS's alphabetical sort is the same as nix's alphabetical sort
...
export const convertToJs = (code)=>{
    ...
    for (const [ parents, node, direction ] of tree.rootNode.traverse()) {
        const isLeafNode = direction == "-"
        if (node.type == "identifier" && node.text == "null") {
            // NULL can't be reassigned so direct translation is fine
        } else if (node.type == "identifier" && (node.text == "true" || node.text == "false")) {
            // true... can be reassigned (its literally just a variable)
        } else if (node.type == "integer_expression") {
            // as-is
            // FIXME: check hex expressions
            ...
            // FIXME: make sure to handle truthy-ness correctly
...
// examples:
    // TODO:
        // dynamic attribute (e.g. ${thing} = 10;) turns into a proxy object
        // record unsafeGetAttrPos for each function/scope/identifier
        // handle path objects (e.g. not just string)
