...
export function nodeList(node) {
    return [ node, ...(node.children||[]).map(nodeList) ].flat(Infinity)
}

export var realParse = (string)=>{
...
export var nodeAsJsonObject = (node)=>{
...
export function echoInputs(path) {
    const tree = nix2json(path)
    let indent = ""
    for (const each of tree.children) {
        if (each.type == "function_expression" && each.children.length) {
            ...
            if (parameterAreas.length > 0 && parameterAreas[0].children.length) {
                const parameterNodes = parameterAreas[0].children
                if (parameterNodes.length && parameterNodes[0].text == "{") {
                    return json2Nix(parameterAreas[0]) + "@inputs: inputs"
                }
            }
        }
    }
