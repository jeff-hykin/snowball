                ...
                while (frontier.length > 0) {
                    ...
                    try {
                        ...
                        if (childNames.length != 0) {
                            ...
                            if (childNames.includes("version")) {
                                uniquePackages[nodeId] = true
                            }
                        }
                    ...
                    if (nodeDepth == worker.nextMaxDepth) {
                        ...
                        if (numberOfNodesProcessed % 1500 == 0) {
                            const numberOfUniquePackages = Object.keys(uniquePackages).length
                            await logLine(`numberOfNodesProcessed:${numberOfNodesProcessed}, spending ${(((new Date()).getTime()-startTime)/numberOfNodesProcessed).toFixed(2)}ms per node, spending ${(((new Date()).getTime()-startTime)/numberOfUniquePackages).toFixed(2)}ms per unique node, # of uniquePackages:${numberOfUniquePackages} currentDepths:\n${JSON.stringify(individualIterCounts)}`)
                            saveNameFrequency()
