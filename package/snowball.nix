...
#     suffix ? "",
#     sha256 ? null,
#     src ? fetchFromGitHub { owner = "NixOS"; repo = "nix"; rev = version; sha256 = sha256; },
#     patches ? [ ]
# }:
...
#                 src             = src;
#                 patches         = patches;
#                 outputs         = [ "out" "dev" ] ++ (if enableDocumentation then [ "man" "doc" ] else []);
#                 hardeningEnable = (if !stdenv.isDarwin then [ "pie" ] else []);

#                 nativeBuildInputs = [ pkg-config ]
#                                     ++ lib.optionals atLeast24 [ autoconf-archive autoreconfHook bison flex jq ]
#                                     ++ lib.optionals (atLeast24 && enableDocumentation) [ (lib.getBin lowdown) mdbook ]
#                                     ++ lib.optionals stdenv.isLinux [ util-linuxMinimal ];
                
#                 buildInputs = [ boost brotli bzip2 curl editline libsodium openssl sqlite xz ]
